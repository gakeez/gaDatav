...
//import { screenData, test } from "../api";

/* eslint-disable-next-line */

const ageMockData = [
  { startValue: 0, value: 131107, axis: "0-20", color: "#467bdb" },
  { startValue: 0, value: 330831, axis: "20-30", color: "#3561b2" },
  { startValue: 0, value: 551238, axis: "30-50", color: "#1e90ff" },
  { startValue: 0, value: 31088, axis: ">50", color: "#8da9dd" },
];
...
export default function() {
  ...
  onMounted(() => {
    task = setInterval(() => {
      todayUser.value += 13;
      growthLastDay.value += 2.7;
      growthLastMonth.value += 1.8;
      averageAge.value++;
      ...
      _ageData.forEach((item) => {
        (item.startValue = item.value),
          (item.value = item.value + random(10000));
      });
      ageData.value = _ageData;

      // 登录设备数据
      const _deviceData = { ...deviceData.value };
      _deviceData.totalDevices += random(10000);
      _deviceData.devices.forEach((item) => {
        item.value += random(10000);
      });
      ...
      realTimeOrderData.value = {
        ...
        data: [...realTimeOrderData.value.data, random(1000)],
      };
    }, 3000);
